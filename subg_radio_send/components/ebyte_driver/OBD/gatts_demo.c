...
static void gatts_profile_a_event_handler(esp_gatts_cb_event_t event, esp_gatt_if_t gatts_if, esp_ble_gatts_cb_param_t *param) {
    ...
    case ESP_GATTS_READ_EVT: {
        ...
        uint8_t tmp = obd_get_engine_speed_val(my_obd);
        printf("speed is %d\n",tmp);
    ...
    case ESP_GATTS_WRITE_EVT: {
        ...
        BaseType_t ret = xQueueSend(my_queue,&combinedNumber,0);
        if (ret == pdPASS)
        {
        ...
        printf("send data is %x,Waiting num is %d,available num is %d\n",combinedNumber,uxQueueMessagesWaiting(my_queue),uxQueueSpacesAvailable(my_queue));
        printf("command is %x\n",combinedNumber);
...
esp_err_t blue_init(){
    ...
    my_queue = xQueueCreate(100,sizeof(uint16_t));
...
BaseType_t  esp_gatts_queue_rec(uint16_t *data){
    BaseType_t ret = xQueueReceive(my_queue,data,0);
    // if (ret == pdPASS)
    // {
    //     printf("rec is success\n");
    // }
    
    // printf("recive data is %x,Waiting num is %d,available num is %d\n",*data,uxQueueMessagesWaiting(my_queue),uxQueueSpacesAvailable(my_queue));
    return ret;
}
