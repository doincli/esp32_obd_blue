...
*
****************************************************************************/
// how to add server according to https://www.esp32.com/viewtopic.php?p=93903
#include "app.h"
...
static uint8_t blue_test_value = 0;
typedef struct {
    uint8_t                 *prepare_buf;
    int                     prepare_len;
} prepare_type_env_t;
...
static const esp_gatts_attr_db_t gatt_db2[HRS_IDX_NB2] =
{
    ...
    /* Characteristic Value */
    [IDX_CHAR_VAL_A2] =
    {{ESP_GATT_RSP_BY_APP}, {ESP_UUID_LEN_16, (uint8_t *)&GATTS_CHAR_UUID_TEST_A2, ESP_GATT_PERM_READ | ESP_GATT_PERM_WRITE,
      GATTS_DEMO_CHAR_VAL_LEN_MAX, sizeof(char_value), (uint8_t *)char_value}},
...
{   
    switch (event) {
        ...
        case ESP_GATTS_READ_EVT:
            ...
            #ifdef SELF_TEST
                if(param->read.handle == handle_start + 10){
                    ESP_LOGI(GATTS_TABLE_TAG, "GATT_READ_EVT, conn_id %d, handle %d", param->read.conn_id,  param->read.handle);   
                }
            #else
                    ESP_LOGI(GATTS_TABLE_TAG, "GATT_READ_EVT, conn_id %d, handle %d", param->read.conn_id,  param->read.handle);    
            #endif
            ...
            uint16_t accuracy_tmp = (uint16_t)(app_get_qam_z_acc()*100);
            if (param->read.handle == handle_start){
                //speed
                rsp.attr_value.len = 1;  
                rsp.attr_value.value[0] = app_obd_get_speed();  
            }else if(param->read.handle == handle_start + 2){
                rsp.attr_value.len = 1;  
                rsp.attr_value.value[0] = (uint8_t)get_rssi();  
                //RSSI
            }else if(param->read.handle == handle_start + 4){
                //z accuracy
                rsp.attr_value.len = 2;  
                rsp.attr_value.value[0] = accuracy_tmp & 0xff;  
                rsp.attr_value.value[1] = (accuracy_tmp >> 8) & 0xff;  
            }else if(param->read.handle == handle_start + 10){
                //z accuracy
                rsp.attr_value.len = 1;  
                rsp.attr_value.value[0] = 0x00;  
            }else
            {
                 ESP_LOGE(GATTS_TABLE_TAG, "unkown cmd\n");  
            }
        ...
        case ESP_GATTS_WRITE_EVT:
            if (!param->write.is_prep){
                #ifdef SELF_TEST
                    if (param->write.handle == handle_start + 10 && param->write.value[0] == 1 ){
                        ESP_LOGI(GATTS_TABLE_TAG, "GATT_WRITE_EVT, handle = %d, value len = %d, value :", param->write.handle, param->write.len);
                        blue_test_value = 1;
                    }
                #else  
                    ESP_LOGI(GATTS_TABLE_TAG, "GATT_WRITE_EVT, handle = %d, value len = %d, value :", param->write.handle, param->write.len);
                    if (param->write.handle == handle_start + 6){
                    //ac11
                    uint8_t number1 = param->write.value[0];  
                    ...
                    if (ret == pdPASS)
                    {
                        ESP_LOGI(GATTS_TABLE_TAG, "queue send success\n");
                        ESP_LOGI(GATTS_TABLE_TAG, "command is %x\n",combinedNumber);
                        ESP_LOGI(GATTS_TABLE_TAG, "send data is %x,Waiting num is %d,available num is %d\n",combinedNumber,uxQueueMessagesWaiting(evt_queue),uxQueueSpacesAvailable(evt_queue));
                    }else if (ret == errQUEUE_FULL)
                    {
                        //队列满处理 可以后面添加
                        ESP_LOGI(GATTS_TABLE_TAG, "queuefull\n");
                    }
                }
                #endif
                /* send response when param->write.need_rsp is true*/
        ...
        case ESP_GATTS_START_EVT:
            //add new  server and characteristic
            if(create_tab1 == false) {
                    handle_start = param->start.service_handle + 2;
...
esp_err_t app_ble_init()
{
    ...
    if (evt_queue == NULL) {
        ESP_LOGE(GATTS_TABLE_TAG, "create queue fail\n");
        return ESP_FAIL;
    }

    esp_err_t ret = blue_init();
    if (ret != ESP_OK){
        ESP_LOGE(GATTS_TABLE_TAG, "init fail\n");
        return ESP_FAIL;
    }
